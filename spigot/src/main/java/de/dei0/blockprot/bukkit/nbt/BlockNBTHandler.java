...
package de.dei0.blockprot.bukkit.nbt;

import de.dei0.blockprot.bukkit.BlockProt;
import de.dei0.blockprot.bukkit.Permissions;
import de.dei0.blockprot.bukkit.nbt.stats.PlayerBlocksStatistic;
import de.dei0.blockprot.bukkit.util.BlockUtil;
import de.dei0.blockprot.nbt.FriendModifyAction;
import de.dei0.blockprot.nbt.LockReturnValue;
...
public final class BlockNBTHandler extends FriendSupportingHandler<NBTCompound> {
     ...
     * <p>
     * <b>Important:</b> After calling this method, you should call
     * {@link #applyToOtherContainer()} to sync the owner data to
     * the other half of double chests/doors.
     *
    ...
    public boolean canAccess(@NotNull final String player) {
        ...
        boolean hasAccess = isNotProtected() || isOwner(player) || (friend.isPresent() && friend.get().canRead());
        
        // If access is denied on this block, check the other half (for double chests/doors)
        // This handles cases where NBT data hasn't fully synced between the two halves
        if (!hasAccess) {
            // Check if this is a double chest
            if (this.block.getType() == Material.CHEST || this.block.getType() == Material.TRAPPED_CHEST) {
                final BlockState doubleChestState = BlockUtil.getDoubleChest(this.block);
                if (doubleChestState != null) {
                    try {
                        final BlockNBTHandler otherHandler = new BlockNBTHandler(doubleChestState.getBlock());
                        Optional<FriendHandler> otherFriend = otherHandler.getFriend(player);
                        return otherHandler.isNotProtected() || otherHandler.isOwner(player) || (otherFriend.isPresent() && otherFriend.get().canRead());
                    } catch (RuntimeException e) {
                        // Other half is not a valid container, continue with original result
                    }
                }
            }
            // Check if this is a door
            else if (BlockProt.getDefaultConfig().isLockableDoor(block.getType())) {
                final Block otherDoor = BlockUtil.getOtherDoorHalf(block.getState());
                if (otherDoor != null) {
                    try {
                        final BlockNBTHandler otherHandler = new BlockNBTHandler(otherDoor);
                        Optional<FriendHandler> otherFriend = otherHandler.getFriend(player);
                        return otherHandler.isNotProtected() || otherHandler.isOwner(player) || (otherFriend.isPresent() && otherFriend.get().canRead());
                    } catch (RuntimeException e) {
                        // Other half is not a valid container, continue with original result
